...
public class BinarySearchTree {
    ...
    // ********************
    // traversal inorder ( left- visit - right)
    public void inorder() {
        inorderRec(root);
        System.out.println();
    }

    // Fungsi rekursif traversal inorder
    private void inorderRec(Node root) {
        if (root != null) {
            inorderRec(root.left);              // Kunjungi kiri
            System.out.print(root.key + " ");   // Cetak node 
            inorderRec(root.right);             // Kunjungi kanan
        }
    }

    // ********************
    // traversal postorder (left - right - visit)
    ...
    // menemukan nilai terkecil dalam sebuah subtree
    private int minValue(Node root) {
        int minv = root.key;
        // ke kiri sampai tidak ada anak kiri lagi
